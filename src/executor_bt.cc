...
ExecutorBT::ExecutorBT()
  ...
  , behavior_name_("")
{
  pnh_.param<std::string>("behavior_tree_package", behavior_package_, "behavior_tree_ros");

  std::string tree_xml;
  if (pnh_.getParam("tree_xml", tree_xml))
  {
    load(tree_xml);
  }
  ...
  pnh_.param("zmq_port", zmq_port_, 0);
  pnh_.param("tree_sleep", tree_sleep_, 100);
}
